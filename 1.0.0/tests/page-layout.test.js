...
export async function runPageLayoutTests() {
  ...
  // 6. 숨김을 되돌릴 수 있다 — 설정을 끄면 가로 스크롤이 복귀해야 한다
  removeAdWingHideStyles(document);
  assert.strictEqual(
    document.querySelectorAll(`#${AD_WING_HIDE_STYLE_ID}`).length,
    0,
    'removeAdWingHideStyles 가 주입된 스타일을 제거하지 못했습니다.'
  );
  removeAdWingHideStyles(document); // 없는 상태에서 다시 불러도 안전
  console.log('✅ [Layout] 날개 숨김을 되돌릴 수 있다 (설정 OFF 시 가로 스크롤 복귀)');

  // 7. 기본값은 숨기지 않는다 — 날개 숨김은 옵션 토글이며 기본 OFF 다
  const { INITIAL_STORAGE_SCHEMA } = await import('../src/core/storage-manager.js');
  assert.strictEqual(
    INITIAL_STORAGE_SCHEMA.settings.hideAdWings,
    false,
    'hideAdWings 기본값이 false 가 아닙니다 — 기본 상태에서 가로 스크롤이 사라집니다.'
  );
  assert.ok(
    /configManager\.get\('hideAdWings'\)/.test(source),
    'src/content/index.js 가 hideAdWings 설정으로 주입을 게이트하지 않습니다.'
  );
  console.log('✅ [Layout] 날개 숨김은 기본 OFF 이며 설정으로 게이트된다');
